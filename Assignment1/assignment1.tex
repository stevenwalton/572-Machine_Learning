...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[grow=down,level distance=3cm]
        \Tree [.{Outlook\\$\big[+9,-6\big]$} 
                [.{Sunny\\$\big[+2,-3\big]$\\\{D1,D2,D8,D9,D11\}} ] 
                [.{Overcast\\$\big[+4,-0\big]$\\\{D3,D7,D12,D13\}} ] 
                [.{Rain\\$\big[+3,-2\big]$\\\{D4,D5,D6,D10,D14\}} ] 
                ]
    \end{tikzpicture}
    \caption{Label of tree with purity and associated days (D)}
\end{figure}
\\
...
is wind. We're actually done on $Sunny$ because we always make a decision to play
tennis or not given our current decision process. This results in the following
...
We now need to do the same thing with Rain. We can skip overcast because it is
pure. Our only decisions left are ``Wind" and ``Temperature". We don't actually
need to know $S_{Rain}$ because we can just figure out which term is the smallest
given Rain.

\begin{align*}
S(Rain,Wind) &= S_{Rain} - S_{Rain,strong} - S_{Rain,weak}\\
              &= S_{Rain} - 0 - 0\\
              &= S_{Rain}
\end{align*}
This is because we have pure outcomes ($S_{Rain,Strong} = \big[+0,-2\big]$, 
$S_{Rain,Weak} = \big[+3,-0\big]$)
We should actually know here that $Wind$ is the variable that we want, but we'll
do $Temperature$ just to check that it isn't equally as good. We know it isn't 
because $S_{Rain,Hot} = \big[+0,-0\big]$, $S_{Rain,Mild} = \big[+2,-1\big]$, and
$S_{Rain,Cold} = \big[+1,-1\big]$, but we'll do it anyway.
\begin{align*}
    S(Rain,Temperature) &= S_{Rain} - S_{Rain,Hot} - S_{Rain,Mild} - S_{Rain,Cold}\\
              &= S_{Rain} - 0 - \frac35(-\frac23\log_2(\frac23) - \frac13\log_2(\frac13)) - \frac25 \\
              &= S_{Rain} - 0.551 - \frac25 \\
              &= S_{Rain} - 0.151
\end{align*}
It is trivial to see that $S_{Rain} > S_{Rain} - 0.151$, so we pick $Wind$. 
We can now complete our tree.
...
We can throw out $Temperature$ because our current decision tree always results
in an outcome of playing tennis or not.
